...
class BaseModel(object):
    ...
    def compute_loss_function(self, raw_data, is_training=None, return_loss_components=False,
        ...
        for model_variable in model_variables:
            regularization_loss += torch.nn.l2_loss(model_variable)
        regularization_loss = self.p.loss.regn * regularization_loss
        
        if self.p.loss.loss_type == 'mse':
            prediction_loss = torch.losses.mean_squared_error(nn_output, processed_data['labels'])
        elif self.p.loss.loss_type == 'l2_loss':
            prediction_loss = torch.nn.l2_loss(nn_output - processed_data['labels'])
        else:
    ...
    def predict_nn_output(self, data, is_training=None):
        ...
        if is_training:
            # Use dropouts
            torch.keras.backend.set_learning_phase(1)
        else:
            # Do not use dropouts
            torch.keras.backend.set_learning_phase(0)
