...
class SBPDMap(ObstacleMap):
    ...
    def dist_to_nearest_obs(self, pos_nk2):
        with torch.name_scope('dist_to_obs'):
    ...
    def get_observation(self, config=None, pos_n3=None, **kwargs):
        ...
        if 'occupancy_grid' in self.p.renderer_params.camera_params.modalities:
            ...
            occupancy_grid_nk2 = torch.reshape(occupancy_grid_world_1mk12, (1, -1, 2))
           
            # Broadcast the occupancy grid to batch size n if needed
            n = pos_n3.shape[0]
            if n != 1:
                occupancy_grid_nk2 = torch.broadcast_to(occupancy_grid_nk2, (n,
            ...
            dist_to_nearest_obs_nmk1 = torch.reshape(dist_to_nearest_obs_nk2, (n, m, k, 1))
            imgs = 0.5 * (1. - torch.sign(dist_to_nearest_obs_nmk1)).numpy()
        else:
