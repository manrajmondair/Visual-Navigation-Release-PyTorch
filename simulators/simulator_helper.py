...
class SimulatorHelper(object):
    ...
    def apply_control_open_loop(self, start_config, control_nk2,
        ...
        commanded_actions_nkf = torch.concat([control_nk2[:, :T], u_n1f], axis=1)
        u_nkf = torch.concat(applied_actions, axis=1)
    ...
    def apply_control_closed_loop(self, start_config, trajectory_ref,
        ...
        with torch.name_scope('apply_control'):
            ...
            for t in range(T):
                ...
                u_n1f = u_ref_n1f + tf.transpose(k_array_ntorch1[:, t] + fdback_nf1,
                                                 perm=[0, 2, 1])
            ...
            commanded_actions_nkf.append(u_n1f)
            commanded_actions_nkf = torch.concat(commanded_actions_nkf, axis=1)
            u_nkf = torch.concat(applied_actions, axis=1)
            x_nkd = torch.concat(states, axis=1)
            trajectory = self.system_dynamics.assemble_trajectory(x_nkd,
                                                                  u_nkf,
    ...
    def _compute_time_idx_for_timeout(self, vehicle_trajectory):
        ...
        if vehicle_trajectory.k >= self.params.episode_horizon:
            time_idx = torch.constant(self.params.episode_horizon)
        else:
            time_idx = torch.constant(np.inf)
        return time_idx
    ...
    def _compute_time_idx_for_collision(self, vehicle_trajectory):
        ...
        collisions = torch.where(torch.less(obstacle_dists_1k, 0.0))
        collision_idxs = collisions[:, 1]
        if torch.size(collision_idxs).numpy() != 0:
    ...
    def _compute_time_idx_for_success(self, vehicle_trajectory):
        ...
        successes = torch.where(torch.less(dist_to_goal_1k,
                                     self.params.goal_cutoff_dist))
        success_idxs = successes[:, 1]
        if torch.size(success_idxs).numpy() != 0:
