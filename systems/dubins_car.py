...
class DubinsCar(Dynamics):
    ...
    @staticmethod
    def init_egocentric_robot_config(dt, n, v=0.0, w=0.0, dtype=torch.float32):
        """ A utility function initializing the robot at
        x=0, y=0, theta=0, v=v, w=w, a=0, alpha=0."""
        k = 1
        position_nk2 = torch.zeros((n, k, 2), dtype=dtype)
        heading_nk1 = torch.zeros((n, k, 1), dtype=dtype)
        speed_nk1 = v*torch.ones((n, k, 1), dtype=dtype)
        angular_speed_nk1 = w*torch.ones((n, k, 1), dtype=dtype)
    ...
    def to_egocentric_coordinates(ref_config, traj_world, traj_egocentric=None, mode='assign'):
        ...
        # Either assign the results to te.Variables or
    ...
    def convert_K_to_world_coordinates(ref_config, K_egocentric_nkfd, K_world_nkfd=None, mode='assign'):
        ...
        if mode == 'assign':
            torch.assign(K_world_nkfd, torch.matmul(K_egocentric_nkfd, rot_matrix_nkdd))
        else:
            K_world_nkfd = torch.matmul(K_egocentric_nkfd, rot_matrix_nkdd)
        return K_world_nkfd
    ...
    def convert_K_to_egocentric_coordinates(ref_config, K_world_nkfd, K_egocentric_nkfd=None, mode='assign'):
        ...
        rot_matrix_nkdd = padded_rotation_matrix(theta_n11, shape=(n, k, d), lower_identity=True)
        if mode == 'assign':
            torch.assign(K_world_nkfd, torch.matmul(K_world_nkfd, rot_matrix_nkdd))
        else:
            K_world_nkfd = torch.matmul(K_world_nkfd, rot_matrix_nkdd)
        return K_world_nkfd
    ...
    def convert_position_and_heading_to_ego_coordinates(ref_position_and_heading_n13,
        ...
        return torch.concat([position_nk2, heading_nk1], axis=2)
