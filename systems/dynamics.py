...
class Dynamics(object):
    ...
    def simulate_T(self, x_n1d, u_nkf, T, pad_mode='zero',
        ...
        for t in range(T):
            ...
            states.append(x_n1d)
        trajectory = self.assemble_trajectory(torch.concat(states, axis=1), u_nkf,
                                              pad_mode=pad_mode)
        return trajectory
    ...
    def _pad_control_vector(self, u_nkf, k, pad_mode=None):
        ...
        if pad_mode == 'zero':  # the last action is 0
            if u_nkf.shape[1]+1 == k:
                u_nkf = torch.concat([u_nkf, torch.zeros((n, 1, self._u_dim))],
                                  axis=1)
            else:
        ...
        # the last action is the same as the second to last action
            ...
            if u_nkf.shape[1]+1 == k:
                u_end_n12 = torch.zeros((n, 1, self._u_dim)) + u_nkf[:, -1:]
                u_nkf = torch.concat([u_nkf, u_end_n12], axis=1)
            else:
                assert(u_nkf.shape[1] == k)
        else:
