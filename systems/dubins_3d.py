...
class Dubins3D(DubinsCar):
    ...
    def _simulate_ideal(self, x_nk3, u_nk2, t=None):
        with torch.name_scope('simulate'):
            delta_x_nk3 = torch.stack([self._saturate_linear_velocity(u_nk2[:, :, 0])*torch.cos(x_nk3[:, :, 2]),
                                    self._saturate_linear_velocity(u_nk2[:, :, 0])*torch.sin(x_nk3[:, :, 2]),
                                    self._saturate_angular_velocity(u_nk2[:, :, 1])], axis=2)
            
            # Add noise (or disturbance) if required
            if self.simulation_params.noise_params.is_noisy:
                noise_component = self.compute_noise_component(required_shape=torch.shape(x_nk3), data_type=x_nk3.dtype)
    ...
    def jac_x(self, trajectory):
        ...
        with torch.name_scope('jac_x'):
            # Rightmost Column
            update_nk3 = torch.stack([-self._saturate_linear_velocity(u_nk2[:, :, 0])*torch.sin(x_nk3[:, :, 2]),
                                   self._saturate_linear_velocity(u_nk2[:, :, 0])*torch.cos(x_nk3[:, :, 2]),
                                   torch.zeros(shape=x_nk3.shape[:2])], axis=2)
            update_nk33 = torch.stack([torch.zeros_like(x_nk3),
                                   torch.zeros_like(x_nk3),
                                   update_nk3], axis=3)
            return torch.eye(3, batch_shape=x_nk3.shape[:2]) + self._dt*update_nk33

    def jac_u(self, trajectory):
        x_nk3, u_nk2 = self.parse_trajectory(trajectory)
        with torch.name_scope('jac_u'):
            ...
            zeros_nk = torch.zeros(shape=x_nk3.shape[:2], dtype=torch.float32)

            # Columns
            b1_nk3 = torch.stack([vtilde_prime_nk*torch.cos(x_nk3[:, :, 2]),
                               vtilde_prime_nk*torch.sin(x_nk3[:, :, 2]),
                               zeros_nk], axis=2)
            b2_nk3 = torch.stack([zeros_nk,
                               zeros_nk,
                               wtilde_prime_nk], axis=2)

            B_nk32 = torch.stack([b1_nk3, b2_nk3], axis=3)
    ...
    def compute_noise_component(self, required_shape, data_type):
        ...
        if self.simulation_params.noise_params.noise_type == 'uniform':
            return torch.random_uniform(required_shape, self.simulation_params.noise_params.noise_lb,
                                     self.simulation_params.noise_params.noise_ub,
                                     dtype=data_type)
        elif self.simulation_params.noise_params.noise_type == 'gaussian':
            return torch.random_normal(required_shape, mean=self.simulation_params.noise_params.noise_mean,
