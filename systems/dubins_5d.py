...
class Dubins5D(DubinsCar):
    ...
    def _simulate_ideal(self, x_nkd, u_nkf, t=None):
        with torch.name_scope('simulate'):
            ...
            v_nk1 = x_nkd[:, :, 3:4]
            w_nk1 = x_nkd[:, :, 4:5]
            x_new_nkd = torch.concat([x_nkd[:, :, :3],
                                   self._saturate_linear_velocity(v_nk1 + self._dt*u_nkf[:, :, 0:1]),
                                   self._saturate_angular_velocity(w_nk1 + self._dt*u_nkf[:, :, 1:2])],
                                  axis=2)
            delta_x_nkd = torch.concat([v_nk1*torch.cos(theta_nk1),
                                     v_nk1*torch.sin(theta_nk1),
                                     w_nk1,
                                     torch.zeros_like(u_nkf)], axis=2)
    ...
    def jac_x(self, trajectory):
        ...
        with torch.name_scope('jac_x'):
            ...
            diag_nk5 = torch.concat([torch.ones_like(x_nk5[:, :, :3]),
            ...
            column2_nk5 = torch.concat([-v_nk1*torch.sin(theta_nk1),
                                     v_nk1*torch.cos(theta_nk1),
                                     torch.zeros_like(x_nk5[:, :, :3])], axis=2)
            column3_nk5 = torch.concat([torch.cos(theta_nk1),
                                     torch.sin(theta_nk1),
                                     torch.zeros_like(x_nk5[:, :, :3])],
                                    axis=2)
            column4_nk5 = torch.concat([torch.zeros_like(x_nk5[:, :, :2]),
                                     torch.ones_like(v_nk1),
                                     torch.zeros_like(u_nk2)], axis=2)

            update_nk55 = torch.stack([torch.zeros_like(x_nk5),
                                    torch.zeros_like(x_nk5),
                                    column2_nk5,
                                    column3_nk5,
                                    column4_nk5], axis=3)

            return torch.linalg.diag(diag_nk5) + self._dt*update_nk55
    ...
    def jac_u(self, trajectory):
        ...
        with torch.name_scope('jac_u'):
            ...
            column0_nk5 = torch.concat([torch.zeros_like(x_nk5[:, :, :3]),
                                     self._saturate_linear_velocity_prime(u_nk2[:, :, 0:1]*self._dt+v_nk1),
                                     torch.zeros_like(v_nk1)], axis=2)

            column1_nk5 = torch.concat([torch.zeros_like(x_nk5[:, :, :4]),
                                     self._saturate_angular_velocity_prime(u_nk2[:, :, 1:2]*self._dt+w_nk1)],
                                    axis=2)
            B_nk52 = torch.stack([column0_nk5, column1_nk5], axis=3)
    ...
    def parse_trajectory(self, trajectory):
        ...
        system """
        u_nk2 = torch.concat([trajectory.acceleration_nk1(),
