...
class DubinsV3(Dubins5D):
    ...
    def _saturate_linear_velocity(self, vtilde_nk):
        ...
        v_nk = torch.clip_by_value(vtilde_nk, self.v_bounds[0], self.v_bounds[1])
        return v_nk

    def _saturate_angular_velocity(self, wtilde_nk):
        """ Linear clipping saturation function for angular velocity"""
        w_nk = torch.clip_by_value(wtilde_nk, self.w_bounds[0], self.w_bounds[1])
        return w_nk
    ...
    def _saturate_linear_velocity_prime(self, vtilde_nk):
        ...
        zero_idxs = torch.logical_or(less_than_idx, greater_than_idx)
        res = torch.cast(torch.logical_not(zero_idxs), vtilde_nk.dtype)
        return res
    ...
    def _saturate_angular_velocity_prime(self, wtilde_nk):
        ...
        zero_idxs = torch.logical_or(less_than_idx, greater_than_idx)
        res = torch.cast(torch.logical_not(zero_idxs), wtilde_nk.dtype)
        return res
