...
class ControlPipelineV0Helper():
    ...
    def compute_closest_waypt_idx(self, desired_waypt_config, waypt_configs):
        ...
        diff = torch.concat([diff_pos_nk2, diff_heading_nk1], axis=2)
        idx = torch.argmin(torch.norm(diff, axis=2))
        return idx.numpy()[0]
    ...
    def load_and_process_data(self, filename, discard_lqr_controller_data=False,
        ...
        if discard_lqr_controller_data:
            ...
            K_nkfd = torch.zeros((2, 1, 1, 1), dtype=np.float32)
            k_nkf1 = torch.zeros((2, 1, 1, 1), dtype=np.float32)
        else:
            ...
            K_nkfd = torch.constant(data['K_nkfd'])
            k_nkf1 = torch.constant(data['k_nkf1'])

        # Load remaining variables
        start_speeds = torch.constant(data['start_speeds'])
        ...
        waypt_configs = SystemConfig.init_from_numpy_repr(track_trajectory_acceleration=track_trajectory_acceleration,
                                                          **data['waypt_configs'])
        horizons = torch.constant(data['horizons'])
    ...
    def gather_across_batch_dim(self, data, idxs):
        ...
        data_bin['start_speeds'] = torch.gather(data['start_speeds'], idxs, axis=0)
        data_bin['spline_trajectories'] = Trajectory.gather_across_batch_dim_and_create(data['spline_trajectories'], idxs)
        data_bin['horizons'] = torch.gather(data['horizons'], idxs, axis=0)
        data_bin['lqr_trajectories'] = Trajectory.gather_across_batch_dim_and_create(data['lqr_trajectories'], idxs)
        data_bin['K_nkfd'] = torch.gather(data['K_nkfd'], idxs, axis=0)
        data_bin['k_nkf1'] = torch.gather(data['k_nkf1'], idxs, axis=0)
        return data_bin
    ...
    def concat_data_across_binning_dim(self, data):
        ...
        data['start_speeds'] = [torch.concat(data['start_speeds'], axis=0)]
        ...
        data['horizons'] = [torch.concat(data['horizons'], axis=0)]
        data['lqr_trajectories'] = [Trajectory.concat_across_batch_dim(data['lqr_trajectories'])]
        data['K_nkfd'] = [torch.concat(data['K_nkfd'], axis=0)]
        data['k_nkf1'] = [torch.concat(data['k_nkf1'], axis=0)]
        return data
