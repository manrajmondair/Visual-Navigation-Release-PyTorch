...
class ControlPipelineV0(ControlPipelineBase):
    ...
    def plan(self, start_config, goal_config=None):
        ...
        idx = torch.squeeze(self._compute_bin_idx_for_start_velocities(start_config.speed_nk1()[:, :, 0])).numpy()
    ...
    def generate_control_pipeline(self, params=None):
        ...
        with torch.name_scope('generate_control_pipeline'):
    ...
    def _dynamically_fit_spline(self, start_config, goal_config):
        ...
        p = self.params
        times_nk = torch.tile(torch.linspace(0., p.planning_horizon_s, p.planning_horizon)[None], [self.waypoint_grid.n, 1])
        final_times_n1 = torch.ones((self.waypoint_grid.n, 1), dtype=torch.float32) * p.planning_horizon_s
        ...
        horizons_n1 = torch.gather(horizons_n1, valid_idxs)
    ...
    def _ensure_world_coordinate_tensors_exist(self, goal_config=None):
        ...
        if _need_to_instantiate_tensors():
            if goal_config is None:
                ...
                if self.params.convert_K_to_world_coordinates:
                    self.Ks_world_nkfd = [torch.Variable(torch.zeros_like(K)) for K in self.K_nkfd]
            else:
                ...
                self.spline_trajectories_world = [Trajectory(dt=dt, n=goal_config.n,
                                                  ...
                                                  track_trajectory_acceleration=self.params.track_trajectory_acceleration)]
                if self.params.convert_K_to_world_coordinates:
                    self.Ks_world_nkfd = [torch.Variable(torch.zeros_like(self.K_nkfd[0][0:1]))]
    ...
    def _rebin_data_by_initial_velocity(self, data):
        ...
        for i in range(len(self.start_velocities)):
            idxs = torch.where(torch.equal(bin_idxs, i))[:, 0]
    ...
    def _compute_bin_idx_for_start_velocities(self, start_speeds_n1):
        ...
        diff = torch.abs(self.start_velocities - start_speeds_n1)
        bin_idxs = torch.argmin(diff, axis=1)
        return bin_idxs
