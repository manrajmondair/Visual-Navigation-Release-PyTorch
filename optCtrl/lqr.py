...
class LQRSolver:
    ...
    def apply_control(self, start_config, trajectory,
        ...
        """
        with torch.name_scope('apply_control'):
            ...
            for t in range(self.T):
                ...
                # TODO: Currently calling numpy() here as te.DEVICE_PLACEMENT_SILENT
                # is not working to place non-gpu ops (i.e. mod) on the cpu
                # turning tensors into numpy arrays is a hack around this.
                error_t_n1d = torch.concat([error_t_n1d[:, :, :angle_dims],
                                         angle_normalize(error_t_n1d[:, :,
                                                                     angle_dims:angle_dims+1].numpy()),
                                         error_t_n1d[:, :, angle_dims+1:]],
                                        axis=2)
                fdback_nf1 = torch.matmul(K_array_nTfd[:, t],
                                       torch.transpose(error_t_n1d, perm=[0, 2, 1]))
                u_n1f = u_ref_n1f + torch.transpose(k_array_nTf1[:, t] + fdback_nf1,
                                                 perm=[0, 2, 1])
                x_next_n1d = self.fwdSim(x_next_n1d, u_n1f, mode=sim_mode)
                actions.append(u_n1f)
                states.append(x_next_n1d)
            u_nkf = torch.concat(actions, axis=1)
            x_nkd = torch.concat(states, axis=1)
    ...
    def back_propagation(self, trajectory):
        ...
        with torch.name_scope('back_prop'):
            angle_dims = self.plant_dyn._angle_dims
            ...
            for t in reversed(range(self.T)):
                ...
                error_t_nd = torch.concat([error_t_nd[:, :angle_dims],
                ...
                dfdx_T_ndd = torch.transpose(dfdx_ndd, perm=[0, 2, 1])
                dfdu_T_ndf = torch.transpose(dfdu_ndf, perm=[0, 2, 1])

                dfdx_T_dot_Vxx_ndd = torch.matmul(dfdx_T_ndd, Vxx_ndd)
                dfdu_T_dot_Vxx_nfd = torch.matmul(dfdu_T_ndf, Vxx_ndd)

                Qx_nd1 = (lqr_sys['dldx_nkd'][:, t][:, :, None] + torch.matmul(dfdx_T_ndd, Vx_nd1)
                          + torch.matmul(dfdx_T_dot_Vxx_ndd, error_t_nd1))
                Qu_nf1 = (lqr_sys['dldu_nkf'][:, t][:, :, None] + torch.matmul(dfdu_T_ndf, Vx_nd1)
                          + torch.matmul(dfdu_T_dot_Vxx_nfd, error_t_nd1))
                Qxx_ndd = lqr_sys['dldxx_nkdd'][:, t] + torch.matmul(dfdx_T_dot_Vxx_ndd, dfdx_ndd)
                Qux_nfd = lqr_sys['dldux_nkfd'][:, t] + torch.matmul(dfdu_T_dot_Vxx_nfd, dfdx_ndd)
                Quu_nff = lqr_sys['dlduu_nkff'][:, t] + torch.matmul(dfdu_T_dot_Vxx_nfd, dfdu_ndf)

                # use regularized inverse for numerical stability
                inv_Quu_nff = self.regularized_pseudo_inverse_(Quu_nff, reg=self.reg)

                # get k and K
                fdfwd_Tnf1[t] = torch.matmul(-inv_Quu_nff, Qu_nf1)
                fdbck_gain_Tnfd[t] = torch.matmul(-inv_Quu_nff, Qux_nfd)
                fdbck_gain_nfd = torch.transpose(fdbck_gain_Tnfd[t], perm=[0, 2, 1])

                # update value function for the previous time step
                Vxx_ndd = Qxx_ndd - torch.matmul(torch.matmul(fdbck_gain_nfd, Quu_nff),
                                              fdbck_gain_Tnfd[t])
                Vx_nd1 = Qx_nd1 - torch.matmul(torch.matmul(fdbck_gain_nfd, Quu_nff), fdfwd_Tnf1[t])

            # Stack the outer time dimension as dimension 1
            # in the tensors
            fdfwd_nTf1 = torch.stack(fdfwd_Tnf1, axis=1)
            fdbck_gain_nTfd = torch.stack(fdbck_gain_Tnfd, axis=1)
    ...
    def build_lqr_system(self, trajectory):
        ...
        dldux_nkfd = torch.transpose(dldxu_nkdf, perm=[0, 1, 3, 2])
    ...
    def regularized_pseudo_inverse_(self, mat, reg=1e-5):
        ...
        """
        if self.inv:
            return torch.matrix_inverse(mat)
        else:
