...
class Trajectory(object):
    ...
    def __init__(self, dt, n, k, position_nk2=None, speed_nk1=None, acceleration_nk1=None, heading_nk1=None,
                 ...
                 dtype=torch.float32, variable=True, direct_init=False,
        ...
        if valid_horizons_n1 is None:
            self.valid_horizons_n1 = torch.ones((n, 1), dtype=torch.float32)*k
        else:
            self.valid_horizons_n1 = torch.constant(valid_horizons_n1)
        ...
        if direct_init:
            ...
            if variable:
                ...
                self._position_nk2 = torch.Variable(torch.zeros([n, k, 2], dtype=dtype) if position_nk2 is None
                                                  else position_nk2)
                self._speed_nk1 = torch.Variable(torch.zeros([n, k, 1], dtype=dtype) if speed_nk1 is None
                                               else torch.constant(speed_nk1, dtype=dtype))
                self._acceleration_nk1 = torch.Variable(torch.zeros([n, k, 1], dtype=dtype) if acceleration_nk1 is None
                                                      else torch.constant(acceleration_nk1, dtype=dtype))

                # Rotational trajectories
                self._heading_nk1 = torch.Variable(torch.zeros([n, k, 1], dtype=dtype) if heading_nk1 is None
                                                 else torch.constant(heading_nk1, dtype=dtype))
                self._angular_speed_nk1 = torch.Variable(torch.zeros([n, k, 1], dtype=dtype) if angular_speed_nk1 is None
                                                       else torch.constant(angular_speed_nk1, dtype=dtype))
                self._angular_acceleration_nk1 = torch.Variable(
                    torch.zeros([n, k, 1], dtype=dtype) if angular_acceleration_nk1 is None
                    else torch.constant(angular_acceleration_nk1, dtype=dtype))
                ...
                # Translational trajectories
                self._position_nk2 = torch.zeros([n, k, 2], dtype=dtype) if position_nk2 is None \
                                                  else torch.constant(position_nk2, dtype=dtype)
                self._speed_nk1 = torch.zeros([n, k, 1], dtype=dtype) if speed_nk1 is None \
                                                  else torch.constant(speed_nk1, dtype=dtype)
                self._acceleration_nk1 = torch.zeros([n, k, 1], dtype=dtype) if acceleration_nk1 is None \
                                                      else torch.constant(acceleration_nk1, dtype=dtype)

                # Rotational trajectories
                self._heading_nk1 = torch.zeros([n, k, 1], dtype=dtype) if heading_nk1 is None \
                                             else torch.constant(heading_nk1, dtype=dtype)
                self._angular_speed_nk1 = torch.zeros([n, k, 1], dtype=dtype) if angular_speed_nk1 is None \
                                                       else torch.constant(angular_speed_nk1, dtype=dtype)
                self._angular_acceleration_nk1 = torch.zeros([n, k, 1], dtype=dtype) if angular_acceleration_nk1 is None \
                    else torch.constant(angular_acceleration_nk1, dtype=dtype)
    ...
    def update_valid_mask_nk(self):
        ...
        all_valid_nk = torch.broadcast_to(torch.range(self.k, dtype=torch.float32)+1, (self.n, self.k))
        self.valid_mask_nk = torch.cast(all_valid_nk <= self.valid_horizons_n1, dtype=torch.float32)
    ...
    def assign_trajectory_from_tensors(self, position_nk2, speed_nk1, acceleration_nk1,
                                       ...
                                       valid_horizons_n1):
        torch.assign(self.position_nk2(), position_nk2)
        torch.assign(self.speed_nk1(), speed_nk1)
        torch.assign(self.acceleration_nk1(), acceleration_nk1)
        torch.assign(self.heading_nk1(), heading_nk1)
        torch.assign(self.angular_speed_nk1(), angular_speed_nk1)
        torch.assign(self.angular_acceleration_nk1(), angular_acceleration_nk1)
    ...
    def gather_across_batch_dim(self, idxs):
        ...
        self._position_nk2 = torch.gather(self._position_nk2, idxs)
        self._speed_nk1 = torch.gather(self._speed_nk1, idxs)
        self._acceleration_nk1 = torch.gather(self._acceleration_nk1, idxs)
        self._heading_nk1 = torch.gather(self._heading_nk1, idxs)
        self._angular_speed_nk1 = torch.gather(self._angular_speed_nk1, idxs)
        self._angular_acceleration_nk1 = torch.gather(self._angular_acceleration_nk1, idxs)
        self.valid_horizons_n1 = torch.gather(self.valid_horizons_n1, idxs)
        return self
    ...
    def concat_across_batch_dim(cls, trajs):
        ...
        position_nk2 = torch.concat([traj.position_nk2() for traj in trajs], axis=0)
        speed_nk1 = torch.concat([traj.speed_nk1() for traj in trajs], axis=0)
        acceleration_nk1 = torch.concat([traj.acceleration_nk1() for traj in trajs], axis=0)
        heading_nk1 = torch.concat([traj.heading_nk1() for traj in trajs], axis=0)
        angular_speed_nk1 = torch.concat([traj.angular_speed_nk1() for traj in trajs], axis=0)
        angular_acceleration_nk1 = torch.concat([traj.angular_acceleration_nk1() for traj in trajs], axis=0)
        valid_horizons_n1 = torch.concat([traj.valid_horizons_n1 for traj in trajs], axis=0)
    ...
    def gather_across_batch_dim_and_create(cls, traj, idxs):
        ...
        position_nk2 = torch.gather(traj.position_nk2(), idxs)
        speed_nk1 = torch.gather(traj.speed_nk1(), idxs)
        acceleration_nk1 = torch.gather(traj.acceleration_nk1(), idxs)
        heading_nk1 = torch.gather(traj.heading_nk1(), idxs)
        angular_speed_nk1 = torch.gather(traj.angular_speed_nk1(), idxs)
        angular_acceleration_nk1 = torch.gather(traj.angular_acceleration_nk1(), idxs)
        valid_horizons_n1 = torch.gather(traj.valid_horizons_n1, idxs)
        return cls(dt=dt, n=n, k=k, position_nk2=position_nk2,
                   speed_nk1=speed_nk1, acceleration_nk1=acceleration_nk1,
                   heading_nk1=heading_nk1, angular_speed_nk1=angular_speed_nk1,
                   angular_acceleration_nk1=angular_acceleration_nk1,
    ...
    def position_and_heading_nk3(self):
        return torch.concat([self.position_nk2(), self.heading_nk1()], axis=2)

    def speed_and_angular_speed_nk2(self):
        return torch.concat([self.speed_nk1(), self.angular_speed_nk1()], axis=2)

    def position_heading_speed_and_angular_speed_nk5(self):
        return torch.concat([self.position_and_heading_nk3(),
    ...
    def append_along_time_axis(self, trajectory):
        ...
        self._position_nk2 = torch.concat([self.position_nk2(),
                                        trajectory.position_nk2()],
                                       axis=1)
        self._speed_nk1 = torch.concat([self.speed_nk1(), trajectory.speed_nk1()],
                                    axis=1)
        self._acceleration_nk1 = torch.concat([self.acceleration_nk1(),
                                            trajectory.acceleration_nk1()],
                                           axis=1)
        self._heading_nk1 = torch.concat([self.heading_nk1(),
                                       trajectory.heading_nk1()], axis=1)
        self._angular_speed_nk1 = torch.concat([self.angular_speed_nk1(),
                                             trajectory.angular_speed_nk1()],
                                            axis=1)
        self._angular_acceleration_nk1 = torch.concat([self.angular_acceleration_nk1(),
    ...
    def clip_along_time_axis(self, horizon):
        ...
        self.k = horizon
        self.valid_horizons_n1 = torch.clip_by_value(self.valid_horizons_n1, 0, horizon)
    ...
    def concat_along_time_axis(cls, trajectories):
        ...
        position_nk2 = torch.concat([x.position_nk2() for x in trajectories], axis=1)
        speed_nk1 = torch.concat([x.speed_nk1() for x in trajectories], axis=1)
        acceleration_nk1 = torch.concat([x.acceleration_nk1() for x in trajectories], axis=1)
        heading_nk1 = torch.concat([x.heading_nk1() for x in trajectories], axis=1)
        angular_speed_nk1 = torch.concat([x.angular_speed_nk1() for x in trajectories], axis=1)
        angular_acceleration_nk1 = torch.concat([x.angular_acceleration_nk1() for x in trajectories], axis=1)
        valid_horizons_n1 = torch.reduce_sum([x.valid_horizons_n1 for x in trajectories], axis=0)
    ...
    def new_traj_clip_along_time_axis(cls, trajectory, horizon,
        ...
        if repeat_second_to_last_speed:
            speed_nk1 = torch.concat([speed_nk1[:, :-1], speed_nk1[:, -2:-1]], axis=1)
            angular_speed_nk1 = torch.concat([angular_speed_nk1[:, :-1],
    ...
    def render(self, axs, batch_idx=0, freq=4, plot_quiver=True, plot_heading=False,
        ...
        if plot_quiver:
            ax.quiver(xs[::freq], ys[::freq],
                      torch.cos(thetas[::freq]), torch.sin(thetas[::freq]))
...
class SystemConfig(Trajectory):
    ...
    def __init__(self, dt, n, k, position_nk2=None, speed_nk1=None, acceleration_nk1=None, heading_nk1=None,
        ...
        super(SystemConfig, self).__init__(dt, n, k, position_nk2, speed_nk1, acceleration_nk1,
                                           ...
                                           angular_acceleration_nk1, dtype=torch.float32,
    ...
    def assign_from_broadcasted_batch(self, config, n):
        ...
        batch size n """
        k = config.k
        self.assign_config_from_tensors(position_nk2=torch.broadcast_to(config.position_nk2(), (n, k, 2)),
                                       speed_nk1=torch.broadcast_to(config.speed_nk1(), (n, k, 1)),
                                       acceleration_nk1=torch.broadcast_to(config.acceleration_nk1(), (n, k, 1)),
                                       heading_nk1=torch.broadcast_to(config.heading_nk1(), (n, k, 1)),
                                       angular_speed_nk1=torch.broadcast_to(config.angular_speed_nk1(), (n, k, 1)),
                                       angular_acceleration_nk1=torch.broadcast_to(config.angular_acceleration_nk1(), (n, k, 1)))
    ...
    def render(self, ax, batch_idx=0, plot_quiver=False, **kwargs):
        ...
        if plot_quiver:
            ax.quiver([pos_3[0]], [pos_3[1]],
                      torch.cos([pos_3[2]]), torch.sin([pos_3[2]]))
