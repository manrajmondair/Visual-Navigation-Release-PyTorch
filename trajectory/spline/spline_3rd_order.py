...
class Spline3rdOrder(Spline):
    ...
    def fit(self, start_config, goal_config, final_times_n1=None, factors=None):
        ...
        if factors is None:  # Compute them heuristically
            factor1_n1 = start_config.speed_nk1()[:, :, 0] + \
                         torch.norm(goal_config.position_nk2()-start_config.position_nk2(), axis=2)
            factor2_n1 = factor1_n1
            factors_n2 = torch.concat([factor1_n1, factor2_n1], axis=1)
        else:
            factors_n2 = factors

        # Compute the final times
        if final_times_n1 is None:
            final_times_n1 = torch.ones((self.n, 1))

        # Fit spline
        with torch.name_scope('fit_spline'):
            ...
            assert(torch.reduce_all(torch.norm(goal_pos_n12-start_pos_n12, axis=2) >=
                                 self.params.epsilon))
            ...
            d1_n1 = x0_n1
            c1_n1 = f1_n1*torch.cos(t0_n1)
            a1_n1 = f2_n1*torch.cos(tg_n1)-2*xg_n1+c1_n1+2*d1_n1
            b1_n1 = 3*xg_n1-f2_n1*torch.cos(tg_n1)-2*c1_n1-3*d1_n1

            d2_n1 = y0_n1
            c2_n1 = f1_n1*torch.sin(t0_n1)
            a2_n1 = f2_n1*torch.sin(tg_n1)-2*yg_n1+c2_n1+2*d2_n1
            b2_n1 = 3*yg_n1-f2_n1*torch.sin(tg_n1)-2*c2_n1-3*d2_n1

            c3_n1 = (final_times_n1 * v0_n1) / f1_n1
            a3_n1 = (final_times_n1*vg_n1/f2_n1) + c3_n1 - 2.
            b3_n1 = 1. - c3_n1 - a3_n1

            self.x_coeffs_n14 = torch.stack([a1_n1, b1_n1, c1_n1, d1_n1], axis=2)
            self.y_coeffs_n14 = torch.stack([a2_n1, b2_n1, c2_n1, d2_n1], axis=2)
            self.p_coeffs_n14 = torch.stack([a3_n1, b3_n1, c3_n1, 0.0*c3_n1],
                                         axis=2)
    ...
    def _eval_spline(self, ts_nk, calculate_speeds=True):
        ...
        with torch.name_scope('eval_spline'):
            ts_n4k = torch.stack([torch.pow(ts_nk, 3), torch.pow(ts_nk, 2),
                               ts_nk, torch.ones_like(ts_nk)], axis=1)
            ps_nk = torch.squeeze(torch.matmul(p_coeffs_n14, ts_n4k), axis=1)

            ps_n4k = torch.stack([torch.pow(ps_nk, 3), torch.pow(ps_nk, 2),
                               ps_nk, torch.ones_like(ps_nk)], axis=1)
            ps_dot_n4k = torch.stack([3.0*torch.pow(ps_nk, 2), 2.0*ps_nk,
                                   torch.ones_like(ps_nk), torch.zeros_like(ps_nk)],
                                  axis=1)

            xs_nk = torch.squeeze(torch.matmul(x_coeffs_n14, ps_n4k), axis=1)
            ys_nk = torch.squeeze(torch.matmul(y_coeffs_n14, ps_n4k), axis=1)

            xs_dot_nk = torch.squeeze(torch.matmul(x_coeffs_n14, ps_dot_n4k), axis=1)
            ys_dot_nk = torch.squeeze(torch.matmul(y_coeffs_n14, ps_dot_n4k), axis=1)

            self._position_nk2 = torch.stack([xs_nk, ys_nk], axis=2)
            self._heading_nk1 = torch.atan2(ys_dot_nk, xs_dot_nk)[:, :, None]

            if calculate_speeds:
                ts_dot_n4k = torch.stack([3.0*torch.pow(ts_nk, 2), 2.0*ts_nk,
                                       torch.ones_like(ts_nk), torch.zeros_like(ts_nk)],
                                      axis=1)
                ps_ddot_n4k = torch.stack([6.0*ps_nk, 2.0*torch.ones_like(ps_nk),
                                        torch.zeros_like(ps_nk),
                                        torch.zeros_like(ps_nk)], axis=1)

                ps_dot_nk = torch.squeeze(torch.matmul(p_coeffs_n14, ts_dot_n4k), axis=1)

                xs_ddot_nk = torch.squeeze(torch.matmul(x_coeffs_n14, ps_ddot_n4k), axis=1)
                ys_ddot_nk = torch.squeeze(torch.matmul(y_coeffs_n14, ps_ddot_n4k), axis=1)

                speed_ps_nk = torch.sqrt(xs_dot_nk**2 + ys_dot_nk**2)
                ...
                self._acceleration_nk1 = torch.zeros_like(self._speed_nk1)
                self._angular_acceleration_nk1 = torch.zeros_like(self._speed_nk1)
    ...
    def check_dynamic_feasibility(self, speed_max_system, angular_speed_max_system, horizon_s):
        ...
        valid_idxs_n = torch.where(required_horizon_n1 <= horizon_s)[:, 0]
        return torch.cast(valid_idxs_n, torch.int32)
    ...
    def compute_dynamically_feasible_horizon(self, speed_max_system, angular_speed_max_system):
        ...
        max_speed_n1 = torch.reduce_max(self.speed_nk1(), axis=1)
        ...
        max_angular_speed_n1 = torch.reduce_max(torch.abs(self.angular_speed_nk1()), axis=1)
        ...
        return torch.maximum(required_horizon_speed_n1, required_horizon_angular_speed_n1)
    ...
    def rescale_spline_horizon_to_dynamically_feasible_horizon(self, speed_max_system,
        ...
        required_horizon_n1 = torch.maximum(required_horizon_n1, minimum_horizon)
        ...
        self.valid_horizons_n1 = torch.ceil(self.final_times_n1/self.dt)
    ...
    def find_trajectories_within_a_horizon(self, horizon_s):
        ...
        """
        valid_idxs_n = torch.where(self.final_times_n1 <= horizon_s)[:, 0]
        return torch.cast(valid_idxs_n, torch.int32)
