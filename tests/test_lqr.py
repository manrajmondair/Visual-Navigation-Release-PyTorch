...
def test_lqr0(visualize=False):
    ...
    x_ref_nk3 = torch.constant(np.tile(goal, (n, k, 1)))
    u_ref_nk2 = torch.constant(np.zeros((n, k, u_dim), dtype=np.float32))
    trajectory_ref = db.assemble_trajectory(x_ref_nk3, u_ref_nk2)

    cost_fn = QuadraticRegulatorRef(trajectory_ref, db, p)

    x_nk3 = torch.constant(np.zeros((n, k, x_dim), dtype=np.float32))
    u_nk2 = torch.constant(np.zeros((n, k, u_dim), dtype=np.float32))
...
def test_lqr1(visualize=False):
    ...
    x_n13 = torch.constant(np.zeros((n, 1, x_dim)), dtype=torch.float32)
    v_1k = np.ones((k-1, 1))*.1
    w_1k = np.linspace(.5, .3, k-1)[:, None]

    u_1k2 = torch.constant(np.concatenate([v_1k, w_1k], axis=1)[None],
                        dtype=torch.float32)
    u_nk2 = torch.zeros((n, k-1, 2), dtype=torch.float32)+u_1k2
    ...
    if visualize:
        ...
        ax.quiver(pos_ref[:, 0], pos_ref[:, 1],
                  torch.cos(heading_ref), torch.sin(heading_ref))
        ax.plot(pos_opt[:, 0], pos_opt[:, 1], 'b-', label='opt')
        ax.quiver(pos_opt[:, 0], pos_opt[:, 1],
                  torch.cos(heading_opt), torch.sin(heading_opt))
        ax.legend()
...
def test_lqr2(visualize=False):
    ...
    goal_x, goal_y = 4.0, 0.0
    goal = np.array([goal_x, goal_y, 0.], dtype=np.float32)
    x_ref_nk3 = torch.constant(np.tile(goal, (1, k, 1)))
    u_ref_nk2 = torch.constant(np.zeros((1, k, u_dim), dtype=np.float32))
    x_nk3 = torch.concat([x_ref_nk3, x_nk3[0:1]], axis=0)
    u_nk2 = torch.concat([u_ref_nk2, u_nk2[0:1]], axis=0)
