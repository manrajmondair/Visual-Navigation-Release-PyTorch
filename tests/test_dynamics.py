...
def test_dubins_v1(visualize=False):
    ...
    state_nk3 = torch.constant(np.zeros((n, k, x_dim), dtype=np.float32))
    ctrl_nk2 = torch.constant(np.random.randn(n, k, u_dim), dtype=np.float32)
    ...
    n, k = 2, 3
    ctrl = 1
    state_n13 = torch.constant(np.zeros((n, 1, x_dim)), dtype=torch.float32)
    ctrl_nk2 = torch.constant(np.ones((n, k, u_dim))*ctrl, dtype=torch.float32)
    ...
    if visualize:
        ...
        k = 50
        state_113 = torch.constant(np.zeros((1, 1, x_dim)), dtype=torch.float32)
        v_1k, w_1k = np.ones((k, 1))*.2, np.linspace(1.1, .9, k)[:, None]
        ctrl_1k2 = torch.constant(np.concatenate([v_1k, w_1k], axis=1)[None],
                               dtype=torch.float32)
...
def test_dubins_v3():
    ...
    state_n15 = torch.constant(np.zeros((n, 1, x_dim)), dtype=torch.float32)
    ctrl_nk2 = torch.constant(np.ones((n, k, u_dim))*ctrl, dtype=torch.float32)
