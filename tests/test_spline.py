...
def test_spline_3rd_order(visualize=False):
    ...
    start_speed_nk1 = torch.ones((n, 1, 1), dtype=torch.float32)*v0

    goal_posx_nk1 = torch.ones((n, 1, 1), dtype=torch.float32) * target_state[0]
    goal_posy_nk1 = torch.ones((n, 1, 1), dtype=torch.float32) * target_state[1]
    goal_pos_nk2 = torch.concat([goal_posx_nk1, goal_posy_nk1], axis=2)
    goal_heading_nk1 = torch.ones((n, 1, 1), dtype=torch.float32) * target_state[2]
    goal_speed_nk1 = torch.ones((n, 1, 1), dtype=torch.float32) * vf
    ...
    ts_nk = torch.tile(torch.linspace(0., dt*k, k)[None], [n, 1])
...
def test_spline_rescaling():
    ...
    dt = .1
    k = 10
    final_times_n1 = torch.constant([[2.], [1.]])
    
    # Goal states and initial speeds
    goal_posx_n11 = torch.constant([[[0.4]], [[1.]]])
    goal_posy_n11 = torch.constant([[[0.]], [[1.]]])
    goal_heading_n11 = torch.constant([[[0.]], [[np.pi/2]]])
    start_speed_nk1 = torch.ones((2, 1, 1), dtype=torch.float32) * 0.5
    ...
    goal_config = SystemConfig(dt, n,
                               k=1,
                               position_nk2=torch.concat([goal_posx_n11, goal_posy_n11], axis=2),
    ...
    ts_nk = torch.stack([torch.linspace(0., final_times_n1[0, 0], 100),
                      torch.linspace(0., final_times_n1[1, 0], 100)], axis=0)
    ...
    max_speed_n1 = torch.reduce_max(spline_trajs.speed_nk1(), axis=1)
    max_angular_speed_n1 = torch.reduce_max(torch.abs(spline_trajs.angular_speed_nk1()), axis=1)
