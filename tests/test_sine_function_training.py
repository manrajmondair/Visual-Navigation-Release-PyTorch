...
def test_sine_function_training():
    ...
    assert np.allclose(torch.nn.l2_loss(trainer_frontend.model.get_trainable_vars()[0]).numpy(), 0.8484554, atol=1e-3)
    assert np.allclose(torch.nn.l2_loss(trainer_frontend.model.get_trainable_vars()[2]).numpy(), 7.455147, atol=1e-3)
