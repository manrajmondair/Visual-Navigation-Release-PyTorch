...
def test_rotate():
    ...
    pos_112 = torch.constant(pos_2[None, None])
    theta_111 = torch.constant(theta_1[None, None])
...
def visualize_coordinate_transform():
    ...
    start_pos_global_n12 = torch.constant([[[1.0, 1.0]]], dtype=torch.float32)
    start_heading_global_n11 = torch.constant([[[np.pi / 2.]]], dtype=torch.float32)
    start_config_global = SystemConfig(dt=dt, n=n, k=1, position_nk2=start_pos_global_n12,
                                       heading_nk1=start_heading_global_n11)

    start_n13 = torch.concat([start_pos_global_n12, start_heading_global_n11], axis=2)
    u_n12 = np.array([[[.01, .1]]], dtype=np.float32)
    u_nk2 = torch.constant(np.broadcast_to(u_n12, (n, k, 2)), dtype=torch.float32)
...
def test_lqr_feedback_coordinate_transform():
    ...
    start_pos_n13 = torch.constant(np.array([[[0.0, 0.0, 0.0]]], dtype=np.float32))
    speed_nk1 = np.ones((n, k - 1, 1), dtype=np.float32) * 2.0
    angular_speed_nk1 = np.linspace(1.5, 1.3, k - 1, dtype=np.float32)[None, :, None]
    u_nk2 = torch.constant(np.concatenate([speed_nk1, angular_speed_nk1], axis=2))
    ...
    start_pos_n13 = torch.constant(np.array([[[1.0, 1.0, np.pi / 2.]]], dtype=np.float32))
    ...
    assert((torch.norm(traj_lqr_world.position_nk2() - traj_test_world.position_nk2(), axis=2).numpy() < 1e-4).all())
    assert((torch.norm(angle_normalize(traj_lqr_world.heading_nk1() - traj_test_world.heading_nk1()), axis=2).numpy() < 1e-4).all())
    assert((torch.norm(traj_lqr_world.speed_nk1() - traj_test_world.speed_nk1(), axis=2).numpy() < 1e-4).all())
    assert((torch.norm(traj_lqr_world.angular_speed_nk1() - traj_test_world.angular_speed_nk1(), axis=2).numpy() < 1e-4).all())
