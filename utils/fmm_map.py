...
class FmmMap(object):
    ...
    def __init__(self, goal_grid_mn, dx=1, map_origin_2=torch.zeros([2], dtype=torch.float32), mask_grid_mn=None):
        """
        Args:
        ...
        self.fmm_distance_map = VoxelMap(scale=dx,
                                         ...
                                         map_size_2=torch.constant([n, m], dtype=torch.float32),
    ...
    def compute_fmm_distance_and_angle(self, mask_value=1000):
        ...
        self.fmm_distance_map.voxel_function_mn = torch.constant(fmm_distance, dtype=torch.float32)
        self.fmm_angle_map.voxel_function_mn = torch.constant(fmm_angle, dtype=torch.float32)
    ...
    def _create_fmm_map_goal_array_mn(goal_positions_n2, map_size_2, dx=1,
                                      map_origin_2=torch.zeros([2], dtype=torch.float32),
                                      mask_grid_mn=None):
