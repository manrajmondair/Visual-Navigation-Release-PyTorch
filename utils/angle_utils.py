...
def rotate_pos_nk2(pos_nk2, theta_n11):
    ...
    pos_rot_nk2 = torch.matmul(rot_matrix_nk22, pos_nk2[:, :, :, None])[:, :, :, 0]
    return pos_rot_nk2
...
def padded_rotation_matrix(theta_n11, shape, lower_identity=False):
    ...
    theta_nk11 = torch.broadcast_to(theta_n11[:, None], (n, k, 1, 1))

    first_row_nkd1 = torch.concat([torch.cos(theta_nk11),
                               torch.sin(theta_nk11),
                               torch.zeros((n, k, d - 2, 1), dtype=dtype)], axis=2)
    second_row_nkd1 = torch.concat([-torch.sin(theta_nk11),
                                torch.cos(theta_nk11),
                                torch.zeros((n, k, d - 2, 1), dtype=dtype)], axis=2)

    # If lower_identity is true, make the lower right
    # e x e matrix the identity matrix
    if lower_identity:
        identity_block_nkee = torch.eye(e, dtype=dtype, batch_shape=(n, k))
        remaining_rows_nkde = torch.concat([torch.zeros((n, k, 2, e), dtype=dtype),
                                         identity_block_nkee], axis=2)
    else:
        remaining_rows_nkde = torch.zeros((n, k, d, d-2), dtype=dtype)
    rot_matrix_nkdd = torch.concat([first_row_nkd1, second_row_nkd1, remaining_rows_nkde], axis=3)
