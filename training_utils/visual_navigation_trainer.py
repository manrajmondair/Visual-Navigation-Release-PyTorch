...
class VisualNavigationTrainer(TrainerFrontendHelper):
    ...
    #         with torch.contrib.summary.always_record_summaries():
    #             torch.contrib.summary.scalar('losses/train', train_loss[-1], step=epoch)
    #             torch.contrib.summary.scalar('losses/validation', validation_loss[-1], step=epoch)
    #
    ...
    #                     torch.contrib.summary.scalar('metrics/{:s}'.format(k.replace(" ", "_")),
    #                                               v, step=epoch)
    ...
    def _init_callback_instance_variables(self):
        ...
        self.nn_summary_writer = torch.contrib.summary.create_file_writer(self._summary_dir(),
    ...
    def test(self):
        ...
        with torch.device(self.p.device):
            simulator_datas = []
