...
def resnet50_cnn(image_size, num_inputs, num_outputs, params, dtype=torch.float32):
    # Input layers
    ...
    with torch.variable_scope('resnet50'):
        ...
        is_training = torch.contrib.eager.Variable(False, dtype=torch.bool, name='is_training')
    ...
    model = torch.keras.Model(inputs=[input_image, input_flat], outputs=x)
