...
Adapted from torch.keras.applications.ResNet50. A notable difference is that the
...
class _IdentityBlock(torch.keras.Model):
...
class _ConvBlock(torch.keras.Model):
...
class ResNet50(torch.keras.Model):
  ...
  def __init__(self,
    ...
    if self.include_top:
      ...
      reduction_indices = torch.constant(reduction_indices)
      if pooling == 'avg':
        ...
        self.global_pooling = functools.partial(
            torch.reduce_max, reduction_indices=reduction_indices, keep_dims=False)
      else:
