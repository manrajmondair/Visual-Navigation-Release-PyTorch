...
class TrainerHelper(object):
    ...
    def train(self, model, data_source, callback_fn=None):
        ...
        for epoch in range(self.p.num_epochs):
            ...
            training_loss_metric = torch.metrics.Mean()
            validation_loss_metric = torch.metrics.Mean()
            ...
            for j in range(0, num_training_samples, self.p.batch_size):
                ...
                self.optimizer.apply_gradients(zip(grads, model.get_trainable_vars()),
                                               global_step=torch.train.get_or_create_global_step())
    ...
    def restore_checkpoint(self, model):
        ...
        """
        # Create a checkpoint
        self.checkpoint = torch.Checkpoint(optimizer=self.create_optimizer(), model=model.arch)
    ...
    def save_checkpoint(self, epoch, model):
        ...
        if not os.path.exists(self.ckpt_dir):
            ...
            self.checkpoint = torch.Checkpoint(optimizer=self.optimizer, model=model.arch)
           
            # Note: This allows the user to specify how many checkpoints should be saved.
            # Tensorflow does not expose the parameter in te.Checkpoint for max_to_keep,
            # however under the hood it uses a Saver object so we can hack around this.
            from torch.python.training.saver import Saver
    ...
    def create_optimizer(self):
        ...
        """
        self.lr = torch.Variable(self.p.lr, dtype=torch.float64)
